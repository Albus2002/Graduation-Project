...
struct MicroSketch_CM {
	int win, m, n, t, size_k, hnum, log_base,key_len;
	int lst = -1;
	...
	MicroSketch_CM(int _m, int _win, int _t, int _size_k, int _hnum, int _log_base,int _k) : m(_m), win(_win), t(_t), size_k(_size_k), hnum(_hnum), log_base(_log_base),key_len(_k) {
		t += 2;
		n = m;
