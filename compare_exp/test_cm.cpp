...
double cal_AAE(map<uint32_t,int> mp,CM_Sketch* sketch,uint64_t timestamp){
	double res = 0;
	int sum = 0;
	for(auto &it:mp){
		sum++;
...
double cal_ARE(map<uint32_t,int> mp, CM_Sketch* sketch,uint64_t timestamp){
	double res = 0;
	int sum = 0;
	for(auto &it:mp){
		sum++;
...
double cal_PR(map<uint32_t,int> mp, CM_Sketch* sketch,uint64_t timestamp,uint32_t threshold){
	double res = 0;
	int real = 0;
	int sum_s = 0;
	for(auto &it:mp){
...
double cal_RR(map<uint32_t,int> mp, CM_Sketch* sketch,uint64_t timestamp,uint32_t threshold){
	double res = 0;
	int real = 0;
	int sum_r = 0;
	for(auto &it:mp){
...
int main(int argc, char *argv[]) {
	freopen("output_cm.txt", "a", stdout);
	...
	int t_mem = mem + int(1 * T * bf_num * bf_rate * M / 1024 / 8);
	if(debug){
        cout<<"mem: "<<mem<<",";
		cout<<"WIN: "<<WIN<<",";
		cout<<"T: "<<T<<",";
	...
	CM_Sketch* sketch = new CM_Sketch(t_mem, w, d, l,key_len);
	double avrg_aae  = 0;
	double avrg_are  = 0;
	double avrg_pr  = 0;
	double avrg_rr  = 0;
