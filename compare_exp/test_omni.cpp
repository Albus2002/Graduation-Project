...
double cal_AAE(map<uint32_t,int> mp,Omni_Window* sketch,uint64_t timestamp){
	double res = 0;
	int sum = 0;
	for(auto &it:mp){
		sum++;
...
double cal_ARE(map<uint32_t,int> mp, Omni_Window* sketch,uint64_t timestamp){
	double res = 0;
	int sum = 0;
	for(auto &it:mp){
		sum++;
...
double cal_PR(map<uint32_t,int> mp, Omni_Window* sketch,uint64_t timestamp,uint32_t threshold){
	double res = 0;
	int real = 0;
	int sum_s = 0;
	for(auto &it:mp){
...
double cal_RR(map<uint32_t,int> mp, Omni_Window* sketch,uint64_t timestamp,uint32_t threshold){
	double res = 0;
	int real = 0;
	int sum_r = 0;
	for(auto &it:mp){
...
int main(int argc, char *argv[]) {
	freopen("output_omni.txt", "a", stdout);
	...
	Omni_Window* sketch = new Omni_Window(t_mem, w, d, l, WIN, s, key_len);
	double avrg_aae  = 0;
	double avrg_are  = 0;
	double avrg_pr  = 0;
	double avrg_rr  = 0;
    int scan = 1;
	for(int i = 0; i < MAXINPUT;++i){
		if(scan % 100 == 0) sketch->insert((uint8_t*)(&item[i]),timestamp[i],1);
